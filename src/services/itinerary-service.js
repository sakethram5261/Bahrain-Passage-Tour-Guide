import { compileLocalItinerary } from './itinerary-database'
import { callLocalAI, buildSpotNarratorPrompt } from './aiService'
import { fetchCmsContent } from './cmsClient'
...
export async function fetchSpotStory(spot) {
  ...
  try {
    ...
    // Fetch fallback/static story from mock CMS groq query
    const cmsData = await fetchCmsContent(`*[_type == 'spot' && id == '${spot.id}'][0]`)
    const fallbackText = cmsData?.story || `Looking upon the majestic site of ${spot.name}, one feels the profound legacy of Dilmun and Islamic history that shaped this coast over millennia.`
    ...
    const cmsData = await fetchCmsContent(`*[_type == 'spot' && id == '${spot?.id}'][0]`).catch(() => null)
    return cmsData?.story || `Looking upon the majestic site of ${spot?.name || 'this landmark'}, one feels the profound legacy of Dilmun and Islamic history that shaped this coast over millennia.`
  }
}
