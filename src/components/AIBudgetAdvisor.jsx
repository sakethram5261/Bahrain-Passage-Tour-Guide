      ...
      <div className={`px-4 py-3 border-b ${headerBg}`}>
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-xl bg-[#C1122F]/5 border border-[#C1122F]/15 text-[#C1122F] hover:bg-[#C1122F]/10 text-xs font-bold uppercase tracking-wider cursor-pointer transition-all font-sans"
          >
        ...
        {daysLeft > 0 && (
          <div className="mt-2.5 flex items-center gap-1.5">
            ...
            <span className="font-sans text-[10px] text-stone-400 shrink-0 font-medium">
      ...
      <div className="px-4 py-3">
        {loading ? (
          <div className="flex items-center gap-2 animate-pulse">
            <div className="h-3 bg-stone-200 rounded flex-1" />
          </div>
        ) : (
          <p className="font-serif text-xs italic text-stone-850 leading-relaxed text-left m-0">
            {tip}
          </p>
        )}
      </div>
        ...
        <div className="border-t border-stone-200 bg-stone-50">
          <div className="p-3 max-h-[160px] overflow-y-auto space-y-2 no-scrollbar">
            {chatMessages.length === 0 && (
              <p className="font-serif text-[12px] italic text-stone-400 text-center py-2">
            ...
            {chatMessages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] px-3 py-2 rounded-xl text-xs leading-relaxed font-sans ${
                  msg.role === 'user'
                    ? 'bg-[#C1122F] text-white rounded-br-sm'
                    : 'bg-white border border-stone-200 text-stone-800 rounded-bl-sm font-serif italic'
                }`}>
                  {msg.text}
                </div>
              </div>
            ...
            {chatLoading && (
              <div className="flex justify-start">
                <div className="px-3 py-2 rounded-xl bg-white border border-stone-200/80 text-[12px] text-stone-400 italic animate-pulse">
                  Thinking...
                </div>
              </div>
            )}
          ...
          <div className="flex gap-2 p-2 border-t border-stone-200 bg-white">
            <input
              value={chatInput}
              onChange={e => setChatInput(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && sendChat()}
              placeholder='E.g. "Can I afford the Four Seasons?"'
              className="flex-1 px-3 py-2 text-xs font-sans rounded-xl border border-stone-200 bg-stone-50 focus:outline-none focus:border-[#C1122F]/40 text-stone-800 placeholder-stone-400"
            />
            <button
              onClick={sendChat}
              disabled={chatLoading || !chatInput.trim()}
              className="px-3 py-2 rounded-xl bg-[#C1122F] text-white text-xs font-bold cursor-pointer transition-all disabled:opacity-40 disabled:cursor-not-allowed border-none font-sans uppercase tracking-wider"
            >
              →
            </button>
          </div>
